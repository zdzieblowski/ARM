...
# based on github: https://github.com/blaz-r/pi_pico_neopixel (2022.07.10)
#
...
class Neopixel:
    ...
    def __init__(self, num_leds, state_machine, pin, mode="RGB", matrix_width = 8, delay=0.0001):
        """
    ...
    def mSetPixel(self, x, y, color):
        self.set_pixel(self.num_leds - ((x + 1) + (y * self.matrix_width)), color)

    def mDrawData(self, data):
        for d in range(len(data)):
            y = math.floor(d / self.matrix_width)
            x = d - y * self.matrix_width
            self.mSetPixel(x, y, data[d])
        self.show()
